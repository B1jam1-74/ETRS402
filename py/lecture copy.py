...
for i in range(270*2):
    for j in range(len(billy[i])):
        if billy[i][j] == 9999:
            del billy[i][j]
        else:
            billy[i][j] = billy[i][j]
#convertir les données en mm
for i in range(270*2):
    for j in range(len(billy[i])):
        # Formula from MATLAB polyfit with exact coefficients
        billy[i][j] = 0.0000000106*billy[i][j]**4 + (-0.0000146535)*(billy[i][j]**3) + 0.0076086045*(billy[i][j]**2) + (-1.9072664930)*(billy[i][j]) + 239.0912491215

#on supprime les valeurs qui s'éloignent le plus de la médianne
...
if billy.size > 0:
    ...
    distances = [val[0] for val in billy]
